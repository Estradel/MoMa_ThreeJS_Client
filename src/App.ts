...
export class App {
  ...
  constructor() {
    ...
    this.globalData = {
      ...
      SESSION_TYPE: [
        SESSION_TYPES.indexOf(String(import.meta.env.VITE_SESSION_TYPE)),
      ],
      API_URL: [
        `http://${String(import.meta.env.VITE_SERVER_IP)}:${String(import.meta.env.VITE_SERVER_PORT)}`,
      ],
      WS_URL: [
        `ws://${String(import.meta.env.VITE_SERVER_IP)}:${String(import.meta.env.VITE_SERVER_PORT)}/ws`,
      ],
      ANIMATIONS: [""],
      ...
      connected: [false],
      cameraFollow: [
        String(import.meta.env.VITE_CAMERA_FOLLOW).toLowerCase() === "true",
      ],
      playbackSpeed: [1],
  ...
  private initLight(): void {
    ...
    const globalLight = new THREE.AmbientLight(); // soft white light
    globalLight.color = new THREE.Color(0xffffff);
    globalLight.intensity = 1.0;
    this.scene.add(globalLight);
    const hemiLight = new THREE.HemisphereLight();
    hemiLight.color = new THREE.Color(0xffffff);
    hemiLight.groundColor = new THREE.Color(0x333333);
    hemiLight.intensity = 5.0;
  ...
  async loadAnimations(): Promise<void> {
    try {
      const response = await sendGetRequest(
        `${this.globalData.API_URL[0]}/animations`,
      );
  ...
  async createSession(): Promise<string> {
    ...
    const sessionInfo = await sendPostRequest(
      ...
      JSON.stringify({
        ...
        animation_file:
          this.globalData.ANIMATIONS[this.globalData.SELECTED_ANIMATION[0]] ||
          "dance1_subject1.bvh",
      }),
    );
  ...
  private animate(_time: number): void {
    ...
    if (this.bvhAnim && this.globalData.cameraFollow[0]) {
      this.controls.target.set(
        this.bvhAnim.bones[0].position.x,
        0,
        this.bvhAnim.bones[0].position.z,
      );
    }
  ...
  async start(): Promise<void> {
    ...
    const autoConnect =
      String(import.meta.env.VITE_AUTO_CONNECT).toLowerCase() === "true";
    if (autoConnect) {
      await this.connectSession();
    }
